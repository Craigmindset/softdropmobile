  ...
  const handlePickAndUploadImage = async () => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      try {
        ...
        // Update carier_profile with new image URL
        const user = (await supabase.auth.getUser()).data.user;
        if (!user) {
          console.log("[Upload] No user found for updating carrier_profile");
          return;
        }
        const { error: updateError } = await supabase
          ...
          .eq("user_id", user.id);
        if (updateError) {
          console.log("[Upload] Error updating carrier_profile:", updateError);
        } else {
          console.log(
            "[Upload] carrier_profile updated with:",
