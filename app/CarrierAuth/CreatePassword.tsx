...
const CreatePassword = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1b. Update display_name in Supabase Auth user profile
      const { error: displayNameError } = await supabase.auth.updateUser({
        data: { display_name: displayName },
      });
      if (displayNameError) {
        alert(displayNameError.message || "Failed to set display name.");
        return;
      }
      ...
      // 3. Upsert user profile in Carrier_profile
      const upsertPayload = {
        user_id,
        phone: phoneNumber,
        password_set: true,
      ...
      // 4. Navigate to success screen and then to login after 3 seconds
      router.replace({
        pathname: "/CarrierAuth/Success",
        params: { phone: phoneNumber, display_name: displayName },
      });
      setTimeout(() => {
        router.replace("/CarrierLogin");
      }, 3000);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() => router.replace("/CarrierLogin")}
      >
